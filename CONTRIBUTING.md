...
DAO Treasury relies on ypricemagic for price lookups. ypricemagic wraps long-running async calls with a DEBUG logger named `y.stuck?`. It only emits when the logger is enabled for DEBUG, and logs "module.function still executing after Xm with args ... kwargs ..." every 5 minutes until completion.
