...
PyInit__matchers(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("dao_treasury__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "dao_treasury__mypyc.ensure_deps");
    ...
    PyObject *capsule = PyObject_GetAttrString(tmp, "init_dao_treasury___sorting____matchers");
    Py_DECREF(tmp);
    if (capsule == NULL) return NULL;
    void *init_func = PyCapsule_GetPointer(capsule, "dao_treasury__mypyc.init_dao_treasury___sorting____matchers");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit__matchers(); }
