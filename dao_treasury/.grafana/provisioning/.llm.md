...
- All dashboard filters (templating variables) **must** query the PostgreSQL server for their list of values.
...
The `${filter:sqlstring}` syntax is supported by Grafana for multi-select variables with the PostgreSQL datasource, be sure to use it.
