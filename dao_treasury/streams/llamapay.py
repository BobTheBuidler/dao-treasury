...
from typing import Awaitable, Callable, Dict, Final, Iterator, List, Optional, Set, final
...
from dao_treasury.db import Stream, StreamedFunds, Address, Token, must_sort_outbound_txgroup_dbid
...
def _generate_dates(start: datetime, end: datetime, stop_at_today: bool = True) -> Iterator[datetime]:
    current = start
    while current < end:
        yield current
...
async def _get_start_timestamp(stream_id: HexStr, block: Optional[BlockNumber] = None) -> int:
...
class LlamaPayProcessor:
    ...
    def streams_for_recipient(self, recipient: ChecksumAddress, at_block: Optional[BlockNumber] = None) -> List[Stream]:
        with db_session:
    ...
    def streams_for_token(self, token: ChecksumAddress, include_inactive: bool = False) -> List[Stream]:
        with db_session:
    ...
    async def process_stream_for_date(self, stream_id: HexStr, date_obj: datetime) -> Optional[StreamedFunds]:
        entity = await _STREAMS_THREAD.run(StreamedFunds.get_entity, stream_id, date_obj)
        if entity:
            return entity
        
        stream_token, start_date = await _STREAMS_THREAD.run(Stream._get_token_and_start_date, stream_id)
        ...
        if start_timestamp == 0:
            ...
            block_datetime = datetime.fromtimestamp(await _get_block_timestamp(block), tz=_UTC)
